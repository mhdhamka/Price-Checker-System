...
<?php
	...
	if (isset($_POST['submit']))
	{
        ...
        if(empty($_FILES['image']['name']))
		{
			...
			{   
				header('location: store.php?editID=1');
			}
			else
			{
		...
		else
		{
			...
			$sql = "UPDATE store
					SET StoreName = '$name', desc1 = '$desc', storeIMG = '$path'
					WHERE storeID = '$storeID'";
			mysqli_query($conn, $sql);
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="store.php?editID=1" class="nav-item nav-link active"><i class="fa fa-store me-2"></i>Store</a>
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
						<h6 class="mb-0">Update Store Details</h6>
                    </div>
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												echo "<tr style = 'text-align: left;'>
														<td><center>".$row['storeID']."</td>
														<td><center>".$row['StoreName']."</td>
														<td>".$row['desc1']."</td>
														<td><center><img src = ".$row['storeIMG']." style = 'width: 100%'></td>
														<td>
															<button class='btn btn-primary'style = 'background-color: white'><a href=\"store.php?editID=$row[storeID]\">Update</a></button>
														</td>
													 </tr>";
											}
										}
					...
					<div class="bg-secondary rounded h-100 p-4">
						...
						<?php
							global $conn;
							$editID = $_GET['editID'];
							$sql = "SELECT * FROM store WHERE storeID = '$editID'";
							$result = mysqli_query($conn, $sql);
						...
						?>	 
                        <div class="row mb-3">
								<label for="inputFname" class="col-sm-2 col-form-label">Store ID</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="inputFname" name = "storeID" value = "<?php echo $storeID ?>" readonly>
								</div>
							</div>
							<div class="row mb-3">
						...
						<div class="row mb-3">
                            ...
                            <div class="row mb-3">
								<label for="inputFname" class="col-sm-2 col-form-label">Store Image</label>
								<div class="col-sm-10">
									<input type="file" class="form-control" id="inputFname" name = "image" value = "<?php echo $img ?>">
								</div>
							</div>
                            
							<button type="submit" class="btn btn-primary" style = "background-color: white"><a href = "store.php?editID=<?php echo $editID ?>" style = "font-color: white;">Cancel</a></button>
